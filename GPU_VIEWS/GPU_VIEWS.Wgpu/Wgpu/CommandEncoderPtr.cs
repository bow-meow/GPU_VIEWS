...
namespace Wgpu
{
    public readonly unsafe struct CommandEncoderPtr
    {
        ...
        public ComputePassEncoderPtr BeginComputePass(ReadOnlySpan<ComputePassTimestampWrites> timestampWrites, string? label = null)
        {
            using var marshalledLabel = new MarshalledString(label, NativeStringEncoding.UTF8);

            var timestampWritesPtr = stackalloc Silk.NET.WebGPU.ComputePassTimestampWrites[timestampWrites.Length];
        ...
        public RenderPassEncoderPtr BeginRenderPass(
            ...
            ReadOnlySpan<RenderPassTimestampWrites> timestampWrites = default, 
            RenderPassDepthStencilAttachment? depthStencilAttachment = null,
            QuerySetPtr? occlusionQuerySet = null,
            string? label = null)
        {
            using var marshalledLabel = new MarshalledString(label, NativeStringEncoding.UTF8);

            var timestampWritesPtr = stackalloc Silk.NET.WebGPU.RenderPassTimestampWrites[timestampWrites.Length];

            for (int i = 0; i < timestampWrites.Length; i++)
                timestampWritesPtr[i] = timestampWrites[i].Pack();

            var colorAttachmentsPtr = stackalloc Silk.NET.WebGPU.RenderPassColorAttachment[colorAttachments.Length];

            for (int i = 0; i < colorAttachments.Length; i++)
                colorAttachmentsPtr[i] = colorAttachments[i].Pack();

            var _depthStencilAttachment = depthStencilAttachment.GetValueOrDefault().Pack();
            ...
            var descriptor = new RenderPassDescriptor
            {
                ...
                DepthStencilAttachment = depthStencilAttachment.HasValue ? &_depthStencilAttachment : null,
                OcclusionQuerySet = occlusionQuerySet.HasValue ? occlusionQuerySet : null
