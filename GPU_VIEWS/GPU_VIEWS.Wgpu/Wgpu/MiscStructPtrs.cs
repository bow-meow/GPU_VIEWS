...
namespace Wgpu
{
    ...
    public readonly unsafe struct SurfacePtr
    {
        ...
        public SurfaceCapabilities GetCapabilities(AdapterPtr adapter)
        {
            var capabilities = new SurfaceCapabilities();
            _wgpu.SurfaceGetCapabilities(_ptr, adapter, ref capabilities);
            return capabilities;
        }

        public SurfaceTexturePtr GetCurrentTexture()
        {
            SurfaceTexture surfaceTexture;
            _wgpu.SurfaceGetCurrentTexture(_ptr, &surfaceTexture);
            return new(_wgpu, &surfaceTexture);
        }

        public void Present()
        {
            _wgpu.SurfacePresent(_ptr);
        }

        public void Configure(SurfaceConfiguration* surfaceConfiguration)
        {
            _wgpu.SurfaceConfigure(_ptr, surfaceConfiguration);
        }
    }

    public readonly unsafe struct SurfaceTexturePtr
    {
        private readonly SurfaceTexture* _ptr;
        public unsafe TexturePtr Texture { get; }
        public Bool32 Suboptimal { get; }
        public SurfaceGetCurrentTextureStatus Status { get; }

        public static implicit operator SurfaceTexture*(SurfaceTexturePtr ptr) => ptr._ptr;

        public SurfaceTexturePtr(WebGPU wgpu, SurfaceTexture* surfaceTexture)
        {
            _ptr = surfaceTexture;
            var unref = *_ptr;
            Texture = new TexturePtr(wgpu, unref.Texture);
            Suboptimal = unref.Suboptimal;
            Status = unref.Status;
        }
    }
    ...
    public readonly unsafe struct TexturePtr
    {
        ...
        public TextureViewPtr CreateView()
        {
            return new(_wgpu, _wgpu.TextureCreateView(_ptr, null));
        }
