...
namespace Wgpu
{
    public readonly unsafe struct RenderPassEncoderPtr
    {
        ...
        {
            uint dyn_offsets = (dynamicOffsets.Length == 0 ? (uint)0 : dynamicOffsets[0]);
            _wgpu.RenderPassEncoderSetBindGroup(_ptr, groupIndex, bindGroup, (uint)dynamicOffsets.Length, in dyn_offsets);
        }
